...
const RollingNumber: React.FC<RollingNumberProps> = ({ targetStr }) => {
  ...
  return (
    <span className="inline-flex overflow-hidden h-12 sm:h-14 font-bold text-4xl sm:text-5xl md:text-6xl text-white tracking-tighter leading-none">
      {/* First Digit: Moves UPWARDS (Increasing) */}
      <span className="relative block h-full overflow-hidden">
        <span
          className="block transition-transform duration-1800 ease-[cubic-bezier(0.16,1,0.3,1)]"
          style={{
            transform: animated
              ? `translateY(-${firstIdx * 10}%)`
              : "translateY(0%)",
          }}
        >
          {digitsUp.map((d, i) => (
            <span key={i} className="block h-12 sm:h-14">
              {d}
            </span>
          ))}
        </span>
  ...
  return (
    <section className="w-full bg-[#040809] text-white py-20 px-4 sm:px-6 md:px-12 lg:px-24 flex flex-col justify-center items-center overflow-hidden antialiased select-none">
      <div className="w-full max-w-5xl mx-auto space-y-12">
        ...
        <div className="relative w-full rounded-[24px] overflow-hidden bg-neutral-900/40 border border-neutral-900 shadow-2xl group aspect-video max-h-130">
          <video
            ref={videoRef}
          ...
          >
            ...
            <div
              className={`p-4 rounded-full bg-neutral-900/80 border border-neutral-800 backdrop-blur-md text-white shadow-2xl transform transition-all duration-300 ${isPlaying ? "opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100" : "opacity-100 scale-100"}`}
            >
              {isPlaying ? (
                <svg
                  className="w-6 h-6"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-4 items-start">
          ...
          <div className="lg:col-span-4 flex flex-col gap-4 w-full">
            ...
            <div className="bg-[#0b1113]/30 border border-neutral-900/80 rounded-[20px] p-5 flex flex-col justify-between items-start h-31.25 relative overflow-hidden backdrop-blur-sm">
              <div className="w-full flex justify-between items-center">
                ...
                <div className="inline-flex items-center gap-1 bg-[#121a1c] border border-neutral-800/60 px-2.5 py-0.5 rounded-full">
                  <span className="text-[10px] font-medium text-neutral-400 tracking-wide flex items-center gap-1">
                    <svg
                      className="w-2.5 h-2.5 text-[#a3e635]"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
          ...
          <div className="lg:col-span-8 space-y-8 lg:pl-6 text-left flex flex-col justify-center h-full pt-2">
            ...
            <p className="text-xl sm:text-2xl md:text-[26px] font-medium text-neutral-200 leading-[1.45] tracking-tight font-sans">
              ...
              engineering—delivering scalable IT ecosystems shaped by technical
              excellence.&rdquo;
            </p>
            ...
            <div className="space-y-1.5 pt-2">
              ...
              <h4
                ...
                style={{
                  fontFamily:
                    "'Brush Script MT', 'Comic Sans MS', cursive, sans-serif",
                  fontStyle: "italic",
                }}
              >
                Amarjeet Chaddhha
