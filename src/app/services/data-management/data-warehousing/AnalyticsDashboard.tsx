export default function AnalyticsDashboard() {
  const kpis = [
    { l: "Data Managed", v: "500+ TB" },
    { l: "Platform Uptime", v: "99.99%" },
    { l: "Reporting Speed", v: "95% Faster" },
    { l: "Active Pipelines", v: "100+" }
  ];
  return (
      ...
      <div className="max-w-5xl mx-auto bg-white p-2 rounded-2xl border border-zinc-200 shadow-sm grid grid-cols-2 md:grid-cols-4 gap-2">
        {kpis.map((k, i) => (
          <div key={i} className="p-4 bg-zinc-50 rounded-xl text-center border border-zinc-100">
            <div className="text-[10px] uppercase font-bold text-zinc-400">{k.l}</div>
            <div className="text-xl font-black text-teal-600 mt-1">{k.v}</div>
          </div>
        ))}
      </div>
    </section>
