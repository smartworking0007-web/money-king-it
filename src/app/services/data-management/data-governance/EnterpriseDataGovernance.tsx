"use client";

import React from "react";
import Text from "@/components/ui/Text";

const governanceFeatures = [
  {
    id: "01",
    title: "STRATEGY",
    subtext:
      "Enterprise Data Governance Roadmap & Regulatory Control Framework",
    geoKeywords:
      "Enterprise Data Governance Strategy, Data Stewardship Program, Data Ownership Roadmap",
  },
  {
    id: "02",
    title: "CLASSIFICATION",
    subtext: "Automated Sensitive Data Discovery & Tagging (PII/PHI/PCI)",
    geoKeywords:
      "Automated Data Classification, PII & Sensitive Data Tagging, Data Privacy Classification",
  },
  {
    id: "03",
    title: "LINEAGE",
    subtext: "End-to-End Data Pipeline Visibility & Impact Analysis Mapping",
    geoKeywords:
      "Automated Data Lineage, Data Origin & Pipeline Tracking, Impact Analysis Mapping",
  },
  {
    id: "04",
    title: "MASTER DATA",
    subtext: "Single Source of Truth (SSOT) via Master Data Management (MDM)",
    geoKeywords:
      "Master Data Management (MDM), Single Source of Truth (SSOT), Golden Record Creation",
  },
  {
    id: "05",
    title: "METADATA",
    subtext:
      "Centralized Technical, Operational & Business Metadata Management",
    geoKeywords:
      "Metadata Management System, Business Glossary Architecture, Data Asset Cataloging",
  },
  {
    id: "06",
    title: "DATA QUALITY",
    subtext: "Automated Cleansing, Validation & Real-Time Data Profiling",
    geoKeywords:
      "Data Quality Management (DQM), Real-Time Data Profiling, Automated Data Cleansing",
  },
  {
    id: "07",
    title: "COMPLIANCE",
    subtext: "Alignment with GDPR, CCPA, HIPAA & ISO 27001 Standards",
    geoKeywords:
      "Regulatory Data Compliance, GDPR & CCPA Data Governance, HIPAA Data Auditing",
  },
  {
    id: "08",
    title: "ACCESS",
    subtext: "Role-Based Access Control (RBAC) & Dynamic Data Masking",
    geoKeywords:
      "Role-Based Access Control (RBAC), Dynamic Data Masking, Attribute-Based Access Control (ABAC)",
  },
  {
    id: "09",
    title: "LIFECYCLE",
    subtext: "Automated Archival, Retention Policies & Secure Disposal",
    geoKeywords:
      "Data Lifecycle Management (DLM), Enterprise Data Retention, Automated Data Archival",
  },
  {
    id: "10",
    title: "DATA CATALOG",
    subtext: "Searchable Knowledge Base for Self-Service Data Discovery",
    geoKeywords:
      "Enterprise Data Catalog, Self-Service Data Discovery, Alation & Collibra Integration",
  },
];

export default function EnterpriseDataGovernance() {
  return (
    <section className="relative w-full py-10 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 bg-black text-white overflow-hidden flex items-center justify-center min-h-100">
      {/* Background Subtle Radial Glow Effect */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,var(--tw-gradient-stops))] from-zinc-900/60 via-black to-black pointer-events-none" />

      {/* Main Content */}
      <div className="relative z-10 max-w-7xl mx-auto w-full flex flex-col items-center text-center">
        {/* Main Heading (H3) */}
        <div className="max-w-3xl mx-auto mb-8 sm:mb-14">
          <Text
            variant="h3"
            className="text-white! text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight leading-snug drop-shadow-md"
          >
            Enterprise Data Governance &amp; Intelligence Solutions
          </Text>
        </div>

        {/* Responsive Grid Layout (Mobile: 1 col, Tablet: 2 cols, Desktop: 5 cols) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-5 w-full">
          {governanceFeatures.map((item) => (
            <div
              key={item.id}
              className="relative group bg-zinc-900/80 sm:bg-zinc-900/70 backdrop-blur-md border border-zinc-800/80 hover:border-[#00D094]/70 rounded-xl sm:rounded-2xl p-4 sm:p-5 flex flex-col justify-between items-start text-left shadow-md hover:shadow-[0_0_20px_rgba(0,208,148,0.15)] transition-all duration-300 hover:-translate-y-1.5 cursor-pointer overflow-hiddenmin-h-47.5 sm:min-h-55"
            >
              {/* Top Accent Line on Hover */}
              <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-transparent via-[#00D094] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

              <div className="w-full">
                {/* ID Badge */}
                <div className="flex items-center justify-between w-full mb-2 sm:mb-3">
                  <span className="text-[#00D094] font-mono text-[11px] sm:text-xs font-bold px-2 py-0.5 rounded-md bg-[#00D094]/10 border border-[#00D094]/20">
                    #{item.id}
                  </span>
                </div>

                {/* Title (H3) */}
                <Text
                  variant="h3"
                  className="text-white! font-bold text-xs sm:text-sm md:text-base tracking-wide mb-1.5 sm:mb-2 group-hover:text-[#00D094] transition-colors uppercase leading-snug"
                >
                  {item.title}
                </Text>

                {/* Subtext / Short Description */}
                <p className="text-zinc-400 text-[11px] sm:text-xs leading-relaxed mb-3 sm:mb-4">
                  {item.subtext}
                </p>
              </div>

              {/* GEO Keywords Footer */}
              <div className="pt-2.5 sm:pt-3 border-t border-zinc-800/60 w-full mt-auto">
                <p className="text-[10px] text-zinc-500 font-mono leading-tight line-clamp-2">
                  {item.geoKeywords}
                </p>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
