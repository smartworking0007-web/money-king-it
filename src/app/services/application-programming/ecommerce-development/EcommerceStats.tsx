...
import { Store, ShieldCheck, BadgeDollarSign, UserCheck } from "lucide-react";
import { Text } from "@/components/ui/Text";

export default function EcommerceStats() {
  const database = [
    { value: "100+", label: "Stores Delivered", icon: Store },
    { value: "99.9%", label: "Uptime Maintained", icon: ShieldCheck },
    { value: "$50M+", label: "Revenue Generated", icon: BadgeDollarSign },
    { value: "95%", label: "Client Retention", icon: UserCheck },
  ];

  return (
    <section className="py-4 w-full text-left">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto px-2">
        {database.map((metric, idx) => (
          <motion.div
            key={idx}
