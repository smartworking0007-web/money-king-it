"use client";

import React from "react";
import Text from "@/components/ui/Text";

const salesforceServices = [
  {
    id: "01",
    title: "CRM DEVELOPMENT",
    subtext:
      "Tailored Apex Architecture, Custom Lightning UI, and Core System Setup",
    geoKeywords:
      "Salesforce Development Services, Apex Code Customization, Enterprise CRM Architecture",
  },
  {
    id: "02",
    title: "LIGHTNING CUSTOM",
    subtext:
      "Bespoke Lightning Web Components (LWC) & User Experience Optimization",
    geoKeywords:
      "Salesforce Lightning Web Components (LWC), Custom Aura Components, Lightning Interface Design",
  },
  {
    id: "03",
    title: "SALES CLOUD",
    subtext:
      "Lead-to-Cash Pipeline Setup, Opportunity Management & Revenue Tracking",
    geoKeywords:
      "Salesforce Sales Cloud Implementation, Lead to Cash Automation, Opportunity Pipeline Management",
  },
  {
    id: "04",
    title: "SERVICE CLOUD",
    subtext:
      "Omnichannel Customer Support, Case Management & Field Service Workflows",
    geoKeywords:
      "Salesforce Service Cloud Setup, Omnichannel Case Management, Customer Service Automation",
  },
  {
    id: "05",
    title: "MARKETING CLOUD",
    subtext:
      "Journey Builder Orchestration, Personalized Email Marketing & Data Cloud Sync",
    geoKeywords:
      "Salesforce Marketing Cloud Implementation, Journey Builder Automation, Customer Data Platform (CDP)",
  },
  {
    id: "06",
    title: "API INTEGRATIONS",
    subtext:
      "Secure REST/SOAP Integrations with ERPs, Billing Engines & External Systems",
    geoKeywords:
      "Salesforce REST API Integration, MuleSoft Salesforce Integration, ERP CRM Middleware Integration",
  },
  {
    id: "07",
    title: "CUSTOM OBJECTS",
    subtext:
      "Specialized Data Modeling, Custom Relationships & Complex Record Architecture",
    geoKeywords:
      "Salesforce Data Modeling, Custom Object Architecture, Lookup & Master-Detail Relationships",
  },
  {
    id: "08",
    title: "CRM MIGRATION",
    subtext:
      "Seamless, High-Fidelity Data Transfer from Legacy Platforms with Zero Downtime",
    geoKeywords:
      "Legacy to Salesforce Migration, HubSpot to Salesforce Migration, CRM Data Cleansing & Import",
  },
  {
    id: "09",
    title: "WORKFLOW AUTOMATION",
    subtext:
      "Advanced Flow Builder Logic, Approval Processes & Automated Alerts",
    geoKeywords:
      "Salesforce Flow Builder Automation, Process Builder Migration, Automated Approval Workflows",
  },
  {
    id: "10",
    title: "APP DEVELOPMENT",
    subtext: "Native AppExchange Package Creation & Enterprise App Publishing",
    geoKeywords:
      "Salesforce AppExchange Development, ISV Partner Solutions, Custom Salesforce App Building",
  },
];

export default function EnterpriseSalesforceSolutions() {
  return (
    <section className="relative w-full py-10 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 bg-black text-white overflow-hidden flex items-center justify-center min-h-100">
      {/* Background Glow Effect */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,var(--tw-gradient-stops))] from-zinc-900/60 via-black to-black pointer-events-none" />

      {/* Main Content */}
      <div className="relative z-10 max-w-7xl mx-auto w-full flex flex-col items-center text-center">
        {/* Main Heading (H3) using Text Typography Component */}
        <div className="max-w-3xl mx-auto mb-8 sm:mb-14">
          <Text
            variant="h3"
            className="text-white! text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight leading-snug drop-shadow-md"
          >
            Enterprise Salesforce Solutions &amp; Custom Consulting
          </Text>
        </div>

        {/* Responsive Grid Layout (Mobile: 1 col, Tablet: 2 cols, Laptop: 3 cols, Desktop: 5 cols) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-5 w-full">
          {salesforceServices.map((item) => (
            <div
              key={item.id}
              className="relative group bg-zinc-900/80 sm:bg-zinc-900/70 backdrop-blur-md border border-zinc-800/80 hover:border-[#00D094]/70 rounded-xl sm:rounded-2xl p-4 sm:p-5 flex flex-col justify-between items-start text-left shadow-md hover:shadow-[0_0_20px_rgba(0,208,148,0.15)] transition-all duration-300 hover:-translate-y-1.5 cursor-pointer overflow-hidden min-h-47.5 sm:min-h-55"
            >
              {/* Top Accent Line on Hover */}
              <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-transparent via-[#00D094] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

              <div className="w-full">
                {/* ID Badge */}
                <div className="flex items-center justify-between w-full mb-2 sm:mb-3">
                  <span className="text-[#00D094] font-mono text-[11px] sm:text-xs font-bold px-2 py-0.5 rounded-md bg-[#00D094]/10 border border-[#00D094]/20">
                    #{item.id}
                  </span>
                </div>

                {/* Title using Text Typography Component */}
                <Text
                  variant="h3"
                  className="text-white! font-bold text-xs sm:text-sm md:text-base tracking-wide mb-1.5 sm:mb-2 group-hover:text-[#00D094] transition-colors uppercase leading-snug"
                >
                  {item.title}
                </Text>

                {/* Subtext Description */}
                <p className="text-zinc-400 text-[11px] sm:text-xs leading-relaxed mb-3 sm:mb-4">
                  {item.subtext}
                </p>
              </div>

              {/* GEO Keywords Footer */}
              <div className="pt-2.5 sm:pt-3 border-t border-zinc-800/60 w-full mt-auto">
                <p className="text-[10px] text-zinc-500 font-mono leading-tight line-clamp-2">
                  {item.geoKeywords}
                </p>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
